Module.register("MMM-YrNow", {
	...
	getScripts: function() {
		return ["printf.js", "readTextFile.js"];
	},
	...
	start: function() {
		this.list = null;
		this.loaded = false;
		var forecastUrl = printf(
			printf("%s", this.config.yrApiUrl),
	...
	getNextPrecipStart: function() {
		return this.list.points.filter(
			item =>
				item.precipitation.intensity > 0 &&
				Date.parse(item.time) >= new Date().valueOf(),
		)[0];
	},
	...
	getMinutesTill: function(nextItemTime) {
		return (
			Math.abs(Date.parse(nextItemTime) - new Date().valueOf()) /
			(1000 * 60)
		);
	},
	...
	getDom: function() {
		...
		if (precipitationStart != null) {
			...
			var precipitationStartsIn = this.getMinutesTill(
				precipitationStart.time,
			);
			...
			if (precipitationStartsIn < 7) {
				...
				if (precipitationStop) {
					precipitationStopsIn = this.getMinutesTill(
						precipitationStop.time,
					);
					nowCast = printf(
						this.translate("precipitation_ends"),
						precipitationStopsIn.toFixed(0),
					);
				} else nowCast = this.translate("precip_next_90");
			} else {
				//Precip in n minutes
				forecast.appendChild(this.getUmbrella());
				nowCast = printf(
					this.translate("precip_in"),
					precipitationStartsIn.toFixed(0),
				);
			}
		}

		if (
			nowCast == this.translate("no_precip_next_90") &&
			this.config.showWeatherForecast
		) {
	...
	createNowcastText: function(nowCast) {
		...
		nowCastText.className = "medium precipText light";
	...
	getTemperature: function() {
		...
		temp.className = "temperature light xlarge bright";
	...
	processForecast: function(obj) {
		if (obj.shortIntervals) {
			this.weatherSymbol = this.calculateWeatherSymbolId(
				obj.shortIntervals[0].symbol,
			);
