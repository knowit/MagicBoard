...
Module.register("MMM-Facial-Recognition-OCV3", {
	...
	showModal: function(message) {
		new Modal({
			...
			onSubmit: value => this.inputCallback(value),
		}).show();
	},
			...
			if (payload.user === -1) {
				...
				if (this.config.welcomeMessage) {
					this.sendNotification(
						...
						this.translate("knownPerson").replace(
							"%person",
							this.current_user,
						),
					);
	...
	notificationReceived: function(notification, payload, sender) {
		...
		if (notification === "KEYPRESS") {
			...
			switch (payload.KeyName) {
				case "ArrowUp":
					self.showModal({
						title: "Do you want to take picture?",
						content: "This is a modal to let you input name",
					});
					break;
				default:
					break;
	...
	getDom: function() {
		const wrapper = this.toggleInputModal();
		return wrapper;
	},
});
