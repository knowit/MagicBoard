...
Module.register("MMM-Ruter", {
		...
		if (this.journeys.length > 0) {
			...
			for (var i = 0; i < this.journeys.length; i++) {
				...
				if (this.config.fade && this.config.fadePoint < 1) {
					...
					var startingPoint =
						this.journeys.length * this.config.fadePoint;
	...
	startPolling: function() {
		...
		for (var i = 0; i < this.config.stops.length; i++) {
			promises.push(
				new Promise(resolv => {
					this.getStopInfo(this.config.stops[i], function(
						err,
						result,
					) {
						resolv(result);
	...
	updateDomIfNeeded: function() {
		...
		for (var i = 0; i < this.journeys.length; i++) {
			...
			if (
				this.previousJourneys[i] == undefined ||
				this.previousJourneys[i].lineName !=
					this.journeys[i].lineName ||
	...
	getStopInfo: function(stopItem, callback) {
		...
		var departureUrl = function() {
			...
			if (stopItem.timeToThere) {
				...
				var timeAhead = moment(moment.now())
					.add(min, "minute")
					.format()
					.substring(0, 16);
				console.log(
					"Looking for journeys " + min + " minutes ahead in time.",
				);
		...
		var stopUrl = function() {
			return "https://reisapi.ruter.no/Place/GetStop/" + stopItem.stopId;
		};
				...
				for (var j = 0; j < stops.length; j++) {
					...
					if (
						...
						if (journey.TrainBlockPart != null) {
							numBlockParts =
								journey.TrainBlockPart.NumberOfBlockParts;
						}

						var stopName = stopItem.stopName
							? stopItem.stopName
							: stop.Name;
						if (self.config.maxNameLength) {
							stopName = stopName.substring(
								0,
								self.config.maxNameLength,
							);
						}
	...
	getTableHeaderRow: function() {
		...
		thLine.className = "line light align-left";
		thLine.appendChild(
			document.createTextNode(this.translate("LINEHEADER")),
		);

		var thDestination = document.createElement("th");
		thDestination.className = "destination light align-left";
		thDestination.appendChild(
			document.createTextNode(this.translate("DESTINATIONHEADER")),
		);

		var thPlatform = document.createElement("th");
		thPlatform.className = "platform light align-left";
		thPlatform.appendChild(
			document.createTextNode(this.translate("PLATFORMHEADER")),
		);

		var thStopName = document.createElement("th");
		thStopName.className = "stopname light align-left";
		thStopName.appendChild(
			document.createTextNode(this.translate("STOPNAMEHEADER")),
		);

		var thTime = document.createElement("th");
		thTime.className = "time light align-left time";
		thTime.appendChild(
			document.createTextNode(this.translate("TIMEHEADER")),
		);
		...
		if (this.config.showStopName) {
			thead.appendChild(thStopName);
		}
		if (this.config.showPlatform) {
			thead.appendChild(thPlatform);
		}
	...
	getTableRow: function(journey) {
		...
		tdDestination.className = "destination bright small";
		tdDestination.appendChild(
			document.createTextNode(journey.destinationName),
		);

		if (this.config.showPlatform) {
			var tdPlatform = document.createElement("td");
			tdPlatform.className = "platform small";
			tdPlatform.appendChild(document.createTextNode(journey.platform));
		}

		if (this.config.showStopName) {
			var tdStopName = document.createElement("td");
			tdStopName.className = "stopname light small";
			tdStopName.appendChild(document.createTextNode(journey.stopName));
		}

		var tdTime = document.createElement("td");
		tdTime.className = "time light small";
		tdTime.appendChild(
			document.createTextNode(this.formatTime(journey.time)),
		);
